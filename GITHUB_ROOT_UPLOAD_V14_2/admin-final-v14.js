...
/* ===== CBT + CLASSES + POSTER FINAL V14 ADMIN ===== */
let adminUser=null;
let ytClasses=[];
let posters=[];
let students=[];
let posterPreviewUrls=[];
const FINAL_ADMIN_TABS=['dashboard','classes','poster','students'];
function byId(id){return document.getElementById(id)}
function tab(name,el){
  FINAL_ADMIN_TABS.forEach(x=>byId(x+'Tab')?.classList.toggle('hidden',x!==name));
  document.querySelectorAll('.simple-sidebar a[data-tab]').forEach(a=>a.classList.remove('active'));
...
function addYoutubePart(id){
  ...
  byId('ytTopic').value=c.topic_key;byId('ytPart').value=next;byId('ytTitle').value=`${basePartTitle(c.class_title||c.topic_name)} - Part ${next}`;byId('ytOrder').value=Number(c.sort_order||0)+1;byId('ytTagline').focus();window.scrollTo({top:0,behavior:'smooth'});toast(`Part ${next} तैयार है—पूरी Tagline और YouTube Link डालें।`,'success');
}
...
async function loadYoutubeClasses(){
  ...
  const r=await sb.from('haryana_youtube_classes').select('*').order('sort_order').order('created_at',{ascending:false});if(r.error){host.innerHTML=`<div class="item text-error">${esc(r.error.message)}<br><small>Final V14 SQL पहले run करें।</small></div>`;return}
...
async function loadPostersAdmin(){
  ...
  const r=await sb.from('app_posters').select('*').order('pinned',{ascending:false}).order('sort_order').order('created_at',{ascending:false});if(r.error){host.innerHTML=`<div class="item text-error">${esc(r.error.message)}<br><small>Final V14 SQL पहले run करें।</small></div>`;return}
...
async function init(){
  if(!(await guard()))return;adminUser=await requireAuth();if(!adminUser)return;if(byId('todayDate'))byId('todayDate').textContent=new Date().toLocaleDateString('hi-IN',{weekday:'long',day:'2-digit',month:'long',year:'numeric'});initInstallUI('adminInstallBtn');fillHaryanaTopics();await Promise.all([loadDashboard(),loadYoutubeClasses(),loadPostersAdmin(),loadStudents()]);
}
init();
