-- =====================================================================
-- GK BY PURUSHOTAM SIR — V12.14 COMPLETE SUPABASE SETUP (FIXED)
-- Run ONLY this file once in Supabase SQL Editor.
--
-- This file replaces the old V12.12 + V12.13 Supabase SQL files.
-- It is safe to rerun after either old script failed part-way.
-- IMPORTANT FIX: no TEMP table is used, so the
-- "relation tmp_gs_5month_plan does not exist" error cannot occur.
-- Existing Auth users, profiles, PDFs, tests, attempts and R2 files are not deleted.
-- =====================================================================

-- ---------- Preflight: base app tables must already exist ----------
do $$
begin
  if to_regclass('public.schedule_days') is null then
    raise exception 'schedule_days table नहीं मिली। पहले app का original Supabase setup चलाएँ।';
  end if;
  if to_regclass('public.daily_targets') is null then
    raise exception 'daily_targets table नहीं मिली। पहले app का original Supabase setup चलाएँ।';
  end if;
  if to_regclass('public.study_materials') is null then
    raise exception 'study_materials table नहीं मिली। पहले app का original Supabase setup चलाएँ।';
  end if;
  if to_regclass('public.tests') is null then
    raise exception 'tests table नहीं मिली। पहले app का original Supabase setup चलाएँ।';
  end if;
end $$;
...
update public.schedule_days set manual_lock=false where manual_lock is null;
update public.schedule_days set manual_unlock=false where manual_unlock is null;
alter table public.schedule_days alter column manual_lock set default false;
alter table public.schedule_days alter column manual_lock set not null;
alter table public.schedule_days alter column manual_unlock set default false;
alter table public.schedule_days alter column manual_unlock set not null;

-- ---------- Flexible class/timing columns ----------
...
-- Normalize any partially-created columns from an older failed run.
update public.daily_targets
set class_status='scheduled'
where class_status is null or class_status not in ('scheduled','partial','completed','cancelled');
update public.daily_targets set is_extra_class=false where is_extra_class is null;
update public.daily_targets set end_time=null
where start_time is not null and end_time is not null and end_time<=start_time;
alter table public.daily_targets alter column class_status set default 'scheduled';
alter table public.daily_targets alter column class_status set not null;
alter table public.daily_targets alter column is_extra_class set default false;
alter table public.daily_targets alter column is_extra_class set not null;
...
create index if not exists daily_targets_schedule_order_idx
on public.daily_targets(schedule_day_id,target_order);
create index if not exists daily_targets_class_status_idx
on public.daily_targets(schedule_day_id,class_status,status);

-- ---------- Expand calendar to 182 days safely ----------
do $$
declare
...
begin
  ...
  if template_row.id is null then
    raise exception 'Base schedule_days नहीं मिले। पहले complete app का original Supabase setup चलाएँ।';
  end if;

  -- Move existing batch dates temporarily to avoid a possible unique(day_date) clash.
  update public.schedule_days
  set day_date=date '2099-01-01'+day_number
  where batch_id=batch_uuid and day_number between 1 and 182;

  for n in 1..182 loop
    if not exists(
      select 1 from public.schedule_days
      where batch_id=batch_uuid and day_number=n
    ) then
      ...
      execute 'insert into public.schedule_days select (jsonb_populate_record(null::public.schedule_days,$1)).*'
      using payload;
    end if;
  end loop;

  -- Day 1 = 03 Aug 2026; Day 182 = 31 Jan 2027 revision calendar.
  update public.schedule_days
  set day_date=date '2026-08-03'+(day_number-1),
      manual_lock=false,
      manual_unlock=false
  where batch_id=batch_uuid and day_number between 1 and 182;
  ...
  where batch_id=batch_uuid and day_number>182;
end $$;

-- ---------- Permanent staging table (not TEMP) ----------
-- Supabase SQL Editor may not preserve a temporary relation between statements.
-- A normal staging table removes that failure and is dropped after the import.
drop table if exists public._v1214_gs_5month_plan_stage;
create table public._v1214_gs_5month_plan_stage(
  day_number integer not null,
  target_order integer not null,
  subject text not null,
  topic text not null,
  primary key(day_number,target_order)
);

insert into public._v1214_gs_5month_plan_stage(day_number,target_order,subject,topic) values
...
from public._v1214_gs_5month_plan_stage p
...
insert into public.daily_targets(
  schedule_day_id,target_order,subject,topic,status,is_required,is_extra_class,class_status
)
...
-- Hide obsolete old planned rows; user-added Extra Classes remain untouched.
update public.daily_targets t
...
where t.schedule_day_id=d.id
  ...
  and not exists(
    select 1 from public._v1214_gs_5month_plan_stage p
    where p.day_number=d.day_number and p.target_order=t.target_order
  );

-- January remains reserved for revision; no invented topics are auto-published.
update public.daily_targets t
set status='draft',class_status='cancelled'
from public.schedule_days d
where t.schedule_day_id=d.id
...
-- Staging relation is no longer needed.
drop table if exists public._v1214_gs_5month_plan_stage;

-- ---------- Flexible class/timing/carry-forward functions ----------
...
-- ---------- Advance-upload and Student visibility controls ----------
do $$
begin
  if not exists(
...
-- Normalize columns if an older attempt created them but did not finish.
update public.daily_targets
set visibility_mode='auto'
where visibility_mode is null or visibility_mode not in ('auto','show','hide');
update public.study_materials set student_visible=false where student_visible is null;
update public.tests set student_visible=false where student_visible is null;
alter table public.daily_targets alter column visibility_mode set default 'auto';
alter table public.daily_targets alter column visibility_mode set not null;
alter table public.study_materials alter column student_visible set default false;
alter table public.study_materials alter column student_visible set not null;
alter table public.tests alter column student_visible set default false;
alter table public.tests alter column student_visible set not null;
...
select
  'V12.14 COMPLETE SETUP SUCCESS' as result,
  count(*) filter (where d.day_number between 1 and 151 and t.status='published') as published_target_rows,
  count(*) filter (where d.day_number between 1 and 5 and t.visibility_mode='show') as first_five_day_visible_rows,
  count(*) filter (where d.day_number between 6 and 151 and t.visibility_mode='auto') as automatic_target_rows
from public.daily_targets t
join public.schedule_days d on d.id=t.schedule_day_id
where d.batch_id='00000000-0000-0000-0000-000000000001';
