...
/* ===== FINAL V16 PREMIUM PASTEL ADMIN ===== */
let adminUser=null;
let ytClasses=[];
let posters=[];
...
function haryanaTopics(){
  ...
  return src.map((t,i)=>({key:t.key||('haryana_'+(i+1)),name:t.name||t.topic_name||t.key,order:i+1}));
}
function canonicalClassGroupKey(key){return CLASS_GROUP_MERGE[key]||key||''}
function canonicalClassGroupName(key,fallback=''){
  const ck=canonicalClassGroupKey(key);if(CLASS_GROUP_NAMES[ck])return CLASS_GROUP_NAMES[ck];
  return haryanaTopics().find(t=>t.key===ck)?.name||fallback||ck;
}
function classPrimaryTopics(){
  const out=[],seen=new Set();for(const t of haryanaTopics()){const key=canonicalClassGroupKey(t.key);if(seen.has(key))continue;seen.add(key);out.push({key,name:canonicalClassGroupName(key,t.name),order:out.length+1})}return out;
}
function classGroupOrder(key){const i=classPrimaryTopics().findIndex(t=>t.key===canonicalClassGroupKey(key));return i>=0?i+1:9999}
function fillHaryanaTopics(){
  const sel=byId('ytTopic');if(!sel)return;
  sel.innerHTML='<option value="">-- Main Topic / Class Group चुनें --</option>'+classPrimaryTopics().map(t=>`<option value="${esc(t.key)}">${String(t.order).padStart(2,'0')}. ${esc(t.name)}</option>`).join('');
  renderCoveredTopicChoices([]);
}
function selectedTopic(){const key=byId('ytTopic')?.value||'';return classPrimaryTopics().find(t=>t.key===key)||null}
function normalizeAdminCovered(c){
  let raw=c?.covered_topics;if(typeof raw==='string'){try{raw=JSON.parse(raw)}catch(_){raw=[]}}if(!Array.isArray(raw))raw=[];
  const all=haryanaTopics(),byKey=new Map(all.map(t=>[t.key,t]));const out=[];
  raw.forEach(x=>{const key=typeof x==='string'?x:String(x?.key||'');if(!key)return;const name=typeof x==='string'?(byKey.get(key)?.name||key):String(x?.name||byKey.get(key)?.name||key);if(!out.some(t=>t.key===key))out.push({key,name})});
  if(!out.length&&c?.topic_key)out.push({key:c.topic_key,name:c.topic_name||byKey.get(c.topic_key)?.name||c.topic_key});return out;
}
function renderCoveredTopicChoices(selectedKeys=[]){
  const host=byId('ytCoveredTopics');if(!host)return;const set=new Set(selectedKeys);
  host.innerHTML=haryanaTopics().map((t,i)=>`<label class="covered-topic-option tone-${(i%6)+1}" data-topic-name="${esc(t.name.toLowerCase())}"><input type="checkbox" value="${esc(t.key)}" ${set.has(t.key)?'checked':''} onchange="updateCoveredTopicsUI()"><span>${esc(t.name)}</span></label>`).join('');updateCoveredTopicsUI(false);
}
function getCoveredTopics(){
  const all=new Map(haryanaTopics().map(t=>[t.key,t]));return [...document.querySelectorAll('#ytCoveredTopics input:checked')].map(x=>all.get(x.value)).filter(Boolean);
}
...
async function saveYoutubeClass(){
  const topic=selectedTopic();if(!topic){setClassFormStatus('Main Topic / Class Group चुनें।','error');return toast('Main Topic चुनें।','error')}
  let covered=getCoveredTopics();if(!covered.length){covered=[haryanaTopics().find(t=>t.key===topic.key)||{key:topic.key,name:topic.name}];renderCoveredTopicChoices(covered.map(x=>x.key))}
  const title=covered.map(x=>x.name).join(' | ')||topic.name;if(byId('ytTitle'))byId('ytTitle').value=title;
  const tagline=String(byId('ytTagline')?.value||'').trim();if(!tagline){setClassFormStatus('पूरी YouTube Tagline लिखें।','error');return toast('YouTube वाली पूरी Tagline लिखें।','error')}
  const youtube=String(byId('ytYoutube')?.value||'').trim();if(!/^https?:\/\//i.test(youtube)){setClassFormStatus('सही YouTube Link दें।','error');return toast('सही YouTube Link दें।','error')}
  const id=byId('ytId')?.value||null;const oldClass=id?ytClasses.find(x=>String(x.id)===String(id)):null;const oldImageKey=oldClass?.image_key||'';const btn=byId('ytSaveBtn');if(btn){btn.disabled=true;btn.textContent='Saving...'}setClassFormStatus('Class save हो रही है…','info');
  try{
    const payload={topic_key:topic.key,topic_name:topic.name,group_key:topic.key,group_name:topic.name,group_order:classGroupOrder(topic.key),covered_topics:covered,class_title:title,part_no:Math.max(1,Number(byId('ytPart')?.value||1)),tagline,youtube_url:youtube,image_key:null,student_visible:!!byId('ytVisible')?.checked,sort_order:classGroupOrder(topic.key)*100+Math.max(1,Number(byId('ytPart')?.value||1)),updated_at:new Date().toISOString()};
    ...
    const r=await q;if(r.error)throw r.error;if(id&&oldImageKey)try{await r2ApiFetch(`/admin/poster?key=${encodeURIComponent(oldImageKey)}`,{method:'DELETE'})}catch(_){ }
    toast(id?'Class update हो गई।':'Class add हो गई।','success');setClassFormStatus('✅ Class सुरक्षित save हो गई।','success');await Promise.all([loadYoutubeClasses(),loadDashboard()]);setTimeout(resetYoutubeForm,900);
  }catch(e){setClassFormStatus((e.message||'Class save नहीं हुई।')+' — Final V16 SQL run हुआ होना चाहिए।','error');toast(e.message||'Class save नहीं हुई।','error')}
...
async function loadPostersAdmin(){
  ...
  const r=await sb.from('app_posters').select('*').order('pinned',{ascending:false}).order('sort_order').order('created_at',{ascending:false});if(r.error){host.innerHTML=`<div class="item text-error">${esc(r.error.message)}<br><small>Final V16 SQL पहले run करें।</small></div>`;return}
...
async function loadImportantInfoAdmin(){
  const host=byId('infoList');if(!host)return;host.innerHTML='<div class="item">Information load हो रही है…</div>';const r=await sb.from('app_important_information').select('*').order('pinned',{ascending:false}).order('sort_order').order('created_at',{ascending:false});if(r.error){host.innerHTML=`<div class="item text-error">${esc(r.error.message)}<br><small>Final V16 SQL run करें।</small></div>`;return}importantInformation=r.data||[];host.innerHTML=importantInformation.map(x=>`<article class="v15-info-row"><div><span class="badge ${x.student_visible?'badge-green':'badge-gray'}">${x.student_visible?'Student Visible':'Hidden'}</span>${x.pinned?'<span class="badge badge-blue">Pinned</span>':''}</div><h3>${esc(x.title||'Important Information')}</h3>${x.message?`<p>${esc(x.message)}</p>`:''}<small>${new Date(x.created_at).toLocaleString('hi-IN')}</small><div class="v15-info-actions"><button class="btn btn-light btn-mini" onclick="editImportantInfo('${x.id}')">Edit</button><button class="btn btn-orange btn-mini" onclick="toggleImportantPin('${x.id}',${!!x.pinned})">${x.pinned?'Unpin':'Pin'}</button><button class="btn btn-blue btn-mini" onclick="toggleImportantInfo('${x.id}',${x.student_visible!==false})">${x.student_visible?'Hide':'Show'}</button><button class="btn btn-red btn-mini" onclick="deleteImportantInfo('${x.id}')">Delete</button></div></article>`).join('')||'<div class="item">अभी कोई Important Information नहीं है।</div>';
}
