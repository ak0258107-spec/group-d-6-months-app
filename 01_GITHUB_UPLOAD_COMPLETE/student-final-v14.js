let user=null,profile=null,ytRows=[],posterRows=[],posterObjectUrls=[];
...
async function loadClasses(){
  ...
  if(r.error){host.innerHTML=`<div class="item text-error">${esc(r.error.message)}<br><small>Final V14 SQL run करें।</small></div>`;return}
  ytRows=r.data||[];
...
async function loadHome(){
  ...
  let weak=0,bookmarks=0,attempts=0,visibleSubjects=0;
  try{
    const [w,b,a,p,s]=await Promise.all([
    ...
    ]);weak=w.count||0;bookmarks=b.count||0;attempts=(a.count||0)+(p.count||0);visibleSubjects=s.count||0;
  }catch(_){ }
  host.innerHTML=`<div class="student-final-hero"><div><span class="simple-kicker">CBT PRACTICE ARENA</span><h2>नमस्ते, ${esc(profile?.full_name||'Student')}</h2><p>CBT लगाइए, Haryana GK Classes देखिए और नए Poster/Announcement यहाँ से खोलिए।</p></div><button class="btn btn-blue" onclick="tab('cbt')">START CBT</button></div><div class="student-stat-grid"><div><b>${visibleSubjects}</b><span>Live Subjects</span></div><div><b>${attempts}</b><span>Attempts</span></div><div><b>${weak}</b><span>Weak Questions</span></div><div><b>${bookmarks}</b><span>Bookmarks</span></div></div><div class="student-home-actions"><a href="cbt-exam-v12-26.html?mode=weak&build=14.0.0">आज मेरे कमजोर प्रश्न कराओ</a><button onclick="tab('revision')">Revision Center</button><button onclick="tab('classes')">Haryana GK Classes</button><button onclick="tab('poster')">Poster / Announcement</button></div>`;
}
async function loadRevision(){
  ...
  host.innerHTML=`<div class="revision-action-grid"><a class="revision-action primary" href="cbt-exam-v12-26.html?mode=weak&build=14.0.0"><b>🎯 कमजोर प्रश्न</b><span>${weak.length} active weak questions</span><small>Wrong + Skipped + Slow questions से revision test</small></a><a class="revision-action" href="cbt-exam-v12-26.html?mode=bookmarks&build=14.0.0"><b>⭐ Bookmarked Questions</b><span>${marks.length} saved</span><small>आपके marked important questions</small></a></div><div class="card" style="margin-top:16px"><h3>Recent Test History</h3><div style="overflow:auto"><table class="data-table"><thead><tr><th>Date</th><th>Q</th><th>Correct</th><th>Wrong</th><th>Accuracy/Score%</th><th>Time</th><th>Mode</th></tr></thead><tbody>${hist||'<tr><td colspan="7">अभी कोई attempt नहीं है।</td></tr>'}</tbody></table></div></div>`;
}
async function renderProfile(){
  const host=byId('profileBox');if(!host)return;let attempts=0,avg=0,best=0;
  const [r,p]=await Promise.all([sb.from('cbt_test_attempts').select('percentage').eq('student_id',user.id),sb.from('cbt_practice_history').select('percentage').eq('student_id',user.id)]);const all=[...(r.data||[]),...(p.data||[])];if(all.length){attempts=all.length;avg=all.reduce((s,x)=>s+Number(x.percentage||0),0)/attempts;best=Math.max(...all.map(x=>Number(x.percentage||0)))}
  host.innerHTML=`<div class="profile-final-card"><img src="${avatarPath(profile?.avatar_gender)}" alt="Avatar"><div><h2>${esc(profile?.full_name||'Student')}</h2><p>${esc(user.email||'')}</p><div class="profile-final-stats"><span><b>${attempts}</b> Attempts</span><span><b>${avg.toFixed(1)}%</b> Average</span><span><b>${best.toFixed(1)}%</b> Best</span></div></div></div><div class="card"><h3>Avatar</h3><div class="profile-avatar-actions"><button onclick="saveAvatar('boy')"><img src="avatar-boy.svg">Boy</button><button onclick="saveAvatar('girl')"><img src="avatar-girl.svg">Girl</button><button onclick="saveAvatar('neutral')"><img src="avatar-student.svg">Student</button></div><button class="btn btn-light" onclick="sendPasswordReset()">Password Reset Email</button><button class="btn btn-red" style="margin-left:8px" onclick="logout()">Logout</button></div>`;
}
async function saveAvatar(gender){const r=await sb.rpc('student_update_avatar_gender',{p_gender:gender});if(r.error)return toast(r.error.message,'error');profile={...(profile||{}),avatar_gender:gender};toast('Avatar save हो गया।','success');renderProfile()}
