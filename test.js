let user,test,questions=[],attempt=null,idx=0,answers={},timerId=null,secondsLeft=0;const __params=new URLSearchParams(location.search),testId=__params.get('id'),returnMode=__params.get('return'),returnMaterial=__params.get('material');
...
async function submitTest(auto=false){
  ...
  if(!pass){
    ...
    resultBox.innerHTML=summary+`<div class="card"><h3>Question-wise Analysis</h3><div class="list">${reviewHtml}</div>${returnMode==='pdf'?'<div class="notice notice-success"><b>✅ PDF Download Unlock हो गई है।</b><p>अब PDF Library में जाकर Download दबाएँ।</p></div>':returnMode==='final'?'<div class="notice notice-success"><b>✅ Final Mock Test Passed</b><p>आज का Target Complete और Verified हो गया है।</p></div>':''}<p><a class="btn btn-blue" href="${returnMode==='pdf'?'l4x8m2r7-k9v3t5n1-z6c4p8q2.html?tab=pdfs':returnMode==='final'?'l4x8m2r7-k9v3t5n1-z6c4p8q2.html?tab=targets':'l4x8m2r7-k9v3t5n1-z6c4p8q2.html'}">${returnMode==='pdf'?'Go to PDF Library':returnMode==='final'?'आज का Target देखें':'Back to Dashboard'}</a></p></div>`;
  }
