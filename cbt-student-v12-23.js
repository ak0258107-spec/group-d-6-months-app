window.__CBT_STUDENT_V1223_LOADED = true;
window.dispatchEvent(new CustomEvent("cbt-student-script-loaded"));
const CBT_DIFFICULTY_FILTER_VERSION = "12.23.0-fast-catalog-single-request";
...
async function initApp() {
    window.__CBT_STUDENT_V1223_READY = false;
    ...
    try {
        const session = await withTimeout(refreshCbtSession(false), 10000, "Login session load नहीं हुई। Student App से दोबारा CBT खोलें।");
        ...
        const quickName = clean(currentAuthUser.user_metadata?.full_name || currentAuthUser.user_metadata?.name || currentAuthUser.email || "Student");
        ...
        const profilePromise = withTimeout(getProfile(currentAuthUser.id), 15000, "Student profile load नहीं हुई।");
        const catalogPromise = loadCatalog();
        const [profileResult] = await Promise.allSettled([profilePromise, catalogPromise]);

        if (profileResult.status === "fulfilled") currentProfile = profileResult.value;
        else throw (profileResult.reason || new Error("Student profile load नहीं हुई।"));

        if (!currentProfile) throw new Error("Student profile नहीं मिली। Admin से account जाँच करवाएँ।");

        if (String(currentProfile.role || "student").toLowerCase() === "admin") {
        ...
        if (nameEl) nameEl.textContent = currentProfile.full_name || currentProfile.student_name || quickName || "Student";
    } catch (error) {
    ...
    window.__CBT_STUDENT_V1223_READY = true;
