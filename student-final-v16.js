...
async function loadHome(){
  ...
  const infoHtml=info.length?`<div class="important-info-stack compact-info">${info.slice(0,3).map(x=>`<article class="important-info-card ${x.pinned?'pinned':''}"><h3>${x.pinned?'★ ':''}${esc(x.title||'Important Information')}</h3>${x.message?`<p>${esc(x.message)}</p>`:''}${x.action_url&&x.action_label?`<a href="${esc(x.action_url)}" target="_blank" rel="noopener">${esc(x.action_label)} →</a>`:''}</article>`).join('')}</div>`:'';
  host.innerHTML=`<div class="home-compact-greeting"><div><b>नमस्ते, ${esc(profile?.full_name||'Student')} 👋</b><span>जहाँ जाना है, सीधे चुनें</span></div></div>${infoHtml}<div class="home-quick-grid"><button class="quick-card quick-cbt" onclick="tab('cbt')"><span class="quick-icon">✓</span><b>CBT Tests</b><small>${visibleSubjects} Live Subject</small></button><button class="quick-card quick-class" onclick="tab('classes')"><span class="quick-icon">▶</span><b>Classes</b><small>Haryana GK</small></button><button class="quick-card quick-poster" onclick="tab('poster')"><span class="quick-icon">▧</span><b>Poster / Updates</b><small>नई जानकारी</small></button><button class="quick-card quick-revision" onclick="tab('revision')"><span class="quick-icon">↻</span><b>Revision</b><small>${weak} Weak • ${bookmarks} Saved</small></button><button class="quick-card quick-profile" onclick="tab('profile')"><span class="quick-icon">●</span><b>Profile</b><small>${attempts} Attempts</small></button><a class="quick-card quick-weak" href="cbt-exam-v12-26.html?mode=weak&build=16.1.0"><span class="quick-icon">🎯</span><b>Weak Questions</b><small>सीधी Practice</small></a></div><div class="student-stat-grid compact-stats"><div><b>${visibleSubjects}</b><span>Subjects</span></div><div><b>${attempts}</b><span>Attempts</span></div><div><b>${weak}</b><span>Weak</span></div><div><b>${bookmarks}</b><span>Saved</span></div></div>`;
}
