...
async function init(){registerSW();user=await requireAuth();if(!user)return;profile=await getProfile(user.id);if(String(profile?.role||'').toLowerCase()==='admin'){location.replace('q9v3x7k2-r8m4p6t1-z5n7c2w9.html');return}await loadCurrentDay();await Promise.all([renderHome(),renderTargets(),loadTests(),loadOneLiners(),loadPdfs(),loadNotifications(),renderProfile()])}
